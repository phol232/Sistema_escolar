...
import { deleteDocenteAction, searchDocentePersonaOptionsAction, upsertDocenteAction } from "@/app/(dashboard)/phase2/actions";
import { StatusBadge } from "@/components/shared/status-badge";
import { useDebounce } from "@/hooks/use-debounce";
import type { DocenteRecord, PaginationMeta, SelectOption } from "@/lib/types/phase2";
import { formatDate } from "@/lib/utils";

import { Activity, BookOpen, CalendarCheck, UserCog } from "lucide-react";
...
function toFormState(docente?: DocenteRecord) {
  return {
...
export function DocentesManager({ docentes, pagination }: DocentesManagerProps) {
  ...
  const [editing, setEditing] = useState<DocenteRecord | null>(null);
  const [deleting, setDeleting] = useState<DocenteRecord | null>(null);
  ...
  useEffect(() => {
    ...
    startPersonaTransition(async () => {
      const options = await searchDocentePersonaOptionsAction({
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      const result = await upsertDocenteAction({
        id: editing?.id ?? null,
        values: form,
      });

      if (!result.ok) {
        setError(result.message ?? "No se pudo guardar el docente.");
        return;
      }
  ...
  function handleDelete() {
    ...
    setDeleteError(null);
    startTransition(async () => {
      const result = await deleteDocenteAction(deleting.id);

      if (!result.ok) {
        setDeleteError(result.message ?? "No se pudo eliminar el docente.");
        return;
      }
  ...
  return (
    <>
      <CrudTable
        ...
        columns={[
          ...
          },
          {
            ...
            icon: BookOpen,
            iconVariant: "purple",
            cell: (row) => row.especialidad || "Sin especialidad",
          },
          {
            header: "Contratación",
            icon: CalendarCheck,
            iconVariant: "green",
            cell: (row) => (row.fecha_contratacion ? formatDate(row.fecha_contratacion) : "Sin fecha"),
          },
          {
            header: "Estado",
            icon: Activity,
        ...
        description="Ficha docente base para asignaciones, tutorías y actividades académicas."
        emptyDescription="Selecciona una persona y conviértela en docente sin mostrar IDs técnicos."
        emptyTitle="No hay docentes registrados"
        onCreate={() => {
          setIsFormOpen(true);
          setEditing(null);
          setPersonaOptions([]);
      ...
      <CrudModal
        description="Asigna la persona por nombre, especialidad principal y fecha de contratación."
        onClose={closeForm}
        open={isFormOpen}
      ...
      >
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <Field label="Especialidad">
            <Input value={form.especialidad} onChange={(event) => setForm((current) => ({ ...current, especialidad: event.target.value }))} />
          </Field>
          <Field label="Fecha de contratación">
            <Input
              type="date"
              value={form.fecha_contratacion}
              onChange={(event) => setForm((current) => ({ ...current, fecha_contratacion: event.target.value }))}
            />
          </Field>
          <SubmitBar error={error} isPending={isPending} onCancel={closeForm} submitLabel={editing ? "Guardar cambios" : "Crear docente"} />
        </form>
      </CrudModal>

      <CrudModal
        description="La eliminación puede bloquearse si el docente ya está vinculado a secciones o cursos."
        onClose={() => {
          setDeleting(null);
          setDeleteError(null);
        }}
      ...
      >
        {deleting ? (
          <DangerBar
            description={`Se eliminará al docente ${deleting.persona_nombre}.`}
            error={deleteError}
            isPending={isPending}
            onCancel={() => {
              setDeleting(null);
